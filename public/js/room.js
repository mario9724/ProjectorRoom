...
const isMKV = currentRoom.sourceUrl && 
  ...
  !currentRoom.sourceUrl.toLowerCase().includes('mp4');
    ...
    if (isMKV) {
      console.log('🚫 Formato MKV detectado - ocultando botón proyectar');
...
// ⭐ FUNCIÓN SIMPLIFICADA (SIN MODAL)
function openInInfuse(streamUrl) {
  const infuseUrl = `infuse://x-callback-url/play?url=${encodeURIComponent(streamUrl)}`;
  console.log('🎬 Abriendo en Infuse:', infuseUrl);
