...
function connectSocket() {
  ...
  // rating-added: reemplaza rating del mismo username
  ...
  // reaction-added: evita duplicado simple (misma user+time+msg)
  socket.on('reaction-added', data => {
    console.log('💬 Reacción añadida:', data);

    const exists = allReactions.some(r =>
      r.username === data.username && r.time === data.time && r.message === data.message
    );
    if (!exists) allReactions.push(data);
...
async function loadHistory() {
  // carga mensajes + ratings + reactions desde el server
  try {
    const [msgRes, ratingRes, reactRes] = await Promise.all([
      fetch(`/api/projectorrooms/${roomId}/messages?limit=200`).then(r => r.json()),
      fetch(`/api/projectorrooms/${roomId}/ratings`).then(r => r.json()),
      fetch(`/api/projectorrooms/${roomId}/reactions`).then(r => r.json())
    ]);

    if (msgRes.success) {
      msgRes.messages.forEach(m => addChatMessage(m.username, m.message, false));
    }

    if (ratingRes.success) {
      allRatings = ratingRes.ratings.map(r => ({ username: r.username, rating: r.rating }));
    }

    if (reactRes.success) {
      allReactions = reactRes.reactions.map(r => ({ username: r.username, time: r.time, message: r.message }));
    }
  } catch (e) {
...
function updateUsersList(users) {
  ...
  if (usersNamesEl) {
    ...
    else if (users.length === 1) usersNamesEl.textContent = `1 roomie en la sala: ${users[0].username}`;
    else {
...
// Antes los vaciabas; ahora los dejamos por compatibilidad
