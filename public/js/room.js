...
async function loadRoomData() {
    ...
    // NUEVO: Cargar historial persistente
    if (data.messages) {
            ...
            <div class="step-card wide">
                <div class="movie-header">
                    ...
                    <div class="movie-info">
                        <h2>${escapeHtml(movieData.title)} • ${movieData.type === 'movie' ? 'Película' : 'Serie'}</h2>
...
async function loadGuestSources(movieData) {
    ...
    try {
        ...
        if (guestSources.length === 0) {
            container.innerHTML = '<div class="loading">⚠️ No se encontraron fuentes disponibles</div>';
            return;
        }
...
function connectSocket() {
    ...
    // NUEVO: Cargar historial al unirse
    socket.on('load-history', (data) => {
        console.log('📚 Cargando historial:', data);
