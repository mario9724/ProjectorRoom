      ...
      <div class="step-card wide">
        <div class="movie-header">
          ...
          <div class="movie-info">
            <h2>${escapeHtml(movieData.title)} ${movieData.episode ? `- ${movieData.episode.season}x${movieData.episode.episode}` : ''}</h2>
...
async function loadGuestSources(movieData) {
  ...
  try {
    ...
    if (movieData.type === 'movie') {
      streamUrl = `${baseUrl}stream/movie/${movieData.imdbId}.json`;
    } else {
      // NUEVO: Soporte para episodios
      if (movieData.episode) {
        streamUrl = `${baseUrl}stream/series/${movieData.imdbId}:${movieData.episode.season}:${movieData.episode.episode}.json`;
      } else {
        streamUrl = `${baseUrl}stream/series/${movieData.imdbId}.json`;
      }
    }
...
function initRoom() {
  ...
  loadRatings();
  loadReactions();
  loadMessages(); // NUEVO: Cargar mensajes persistentes
...
function renderRoom() {
  ...
  if (titleEl) {
    ...
    if (movieData.episode) {
      titleText += ` - ${movieData.episode.season}x${movieData.episode.episode}`;
    }
  ...
  if (typeEl) {
    let typeText = movieData.type === 'movie' ? 'Película' : 'Serie';
    if (movieData.episode) {
      typeText += ` - ${movieData.episode.name}`;
    }
    typeEl.textContent = typeText;
  }
  if (ratingEl) ratingEl.textContent = '⭐ ' + (movieData.rating || 'N/A');
  if (overviewEl) {
    const overview = movieData.episode ? movieData.episode.overview : movieData.overview;
    overviewEl.textContent = overview || 'Sin descripción disponible';
  }
  ...
  document.getElementById('btnSubmitRating').onclick = async function() {
    ...
    try {
      const res = await fetch(`/api/projectorrooms/${roomId}/ratings`, {
        ...
        body: JSON.stringify({ username, rating: selectedRating })
      });

      const data = await res.json();
      
      if (data.success && socket && roomId) {
...
async function submitReaction() {
  ...
  try {
    const res = await fetch(`/api/projectorrooms/${roomId}/reactions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username, time, message })
    });
...
// NUEVO: Cargar datos persistentes al iniciar
