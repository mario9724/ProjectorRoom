...
let selectedMovie = null; // { tmdbId, type: 'movie'|'series', imdbId, title, poster, rating, year, overview }
let sources = [];
...
let roomConfig = {
  username: '',
  roomName: '',
  projectorType: 'public', // public|custom
  customManifest: '',
  shareMode: 'host' // host|guest
};
...
function selectProjectorType(type) {
  ...
  document.querySelectorAll('#step3 .option-card').forEach(card => card.classList.remove('selected'));
...
function renderMovieGrid(movies) {
  ...
  movies.forEach(movie => {
    ...
    card.innerHTML = `
      <img src="https://image.tmdb.org/t/p/w500${movie.poster_path}" alt="${escapeHtml(title)}">
...
async function selectMovie(movie) {
  ...
  // Obtener IMDb ID (necesario para streams)
  try {
    ...
    console.error('Error obteniendo IMDb ID:', err);
...
function renderSelectedMovie() {
  ...
  document.getElementById('selectedYear').textContent = selectedMovie.year || 'N/A';
...
async function createRoom() {
  ...
  // Guardar SOLO IDs (la UI de sala se rellena desde TMDB)
  const manifestIds = {
    tmdbId: selectedMovie.tmdbId,
    type: selectedMovie.type,   // movie|series
  ...
  const payload = {
    ...
    manifest: JSON.stringify(manifestIds),
  ...
  try {
    ...
    const data = await res.json();
    if (!data.success) {
      alert(data.message || 'Error creando sala');
      return;
    }
